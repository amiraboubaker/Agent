import { PenTool, Clock3, Plus } from "lucide-react";
import type { Conversation } from "../types";

export function Sidebar({ conversations, onNew }: { conversations: Conversation[]; onNew: () => void }) {
    return <aside className="sidebar">
        <div className="brand"><span className="brand-mark"><PenTool size={19} /></span><span>studio<span className="brand-dot">.</span></span></div>
        <button className="new-chat-button" onClick={onNew} type="button"><Plus size={17} /> New conversation</button>
        <div className="history-heading"><span>Recent designs</span><Clock3 size={14} /></div>
