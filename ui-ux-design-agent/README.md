# Studio: UI/UX Design Agent

Studio is a full-stack UI/UX design assistant with a React + TypeScript interface and a FastAPI backend. It classifies requests as UX research, user flows, information architecture, UI design, or accessibility, persists conversations in SQLite, and sends prompts to a configurable OpenAI-compatible chat-completions API.

Design guidance is displayed as Markdown with syntax highlighting and can be copied. The application does not claim that research or usability validation was performed without evidence.
