...
export function Sidebar({ conversations, onNew }: { conversations: Conversation[]; onNew: () => void }) {
    return <aside className="sidebar">
        <div className="brand"><span className="brand-mark"><BarChart3 size={19} /></span><span>insight<span className="brand-dot">.</span></span></div>
        <button className="new-chat-button" onClick={onNew} type="button"><Plus size={17} /> New conversation</button>
        <div className="history-heading"><span>Recent analyses</span><Clock3 size={14} /></div>
