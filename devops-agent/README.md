# Ops: DevOps and Cloud Infrastructure Agent

Ops is a full-stack DevOps and cloud infrastructure assistant with a React + TypeScript interface and a FastAPI backend. It classifies requests as deployment, troubleshooting, automation, security, monitoring, or general infrastructure, persists conversations in SQLite, and sends prompts to a configurable OpenAI-compatible chat-completions API.

Infrastructure guidance is displayed as Markdown with syntax highlighting and can be copied. The application never executes generated commands automatically, never assumes a command succeeded, and never exposes credentials.
...
For production, serve `frontend/dist` from a static host or Nginx and run the FastAPI service behind a production ASGI process such as Gunicorn with Uvicorn workers. Set a specific production `CORS_ORIGINS`, use a managed database when needed, and inject `LLM_API_KEY` through the deployment secret manager. Treat generated commands as reviewable runbooks and validate each command in its stated environment before continuing.
...
- Generated commands are never executed by the service.
- Review model output and target environment before changing infrastructure.
