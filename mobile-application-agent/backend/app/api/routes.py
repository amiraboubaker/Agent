...
from app.services.conversations import create_conversation, get_conversation, list_conversations, update_conversation
...
def health() -> dict[str, str]:
    return {"status": "ok", "service": "mobile-application-agent"}
...
async def chat(http_request: Request, request: ChatRequest, settings: Settings = Depends(get_settings)) -> ChatResponse:
    ...
    return ChatResponse(conversation_id=conversation_id, task_type=task_type, answer=answer, created_at=datetime.now(UTC))
