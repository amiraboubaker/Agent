    ...
    return (
        <article className={`message-row ${isUser ? "message-row-user" : ""}`}>
            ...
            <div className="message-content">
                <div className="message-meta"><strong>{isUser ? "You" : "Product Management Agent"}</strong>{message.taskType && <span className="task-chip">{message.taskType.replace("_", " ")}</span>}</div>
